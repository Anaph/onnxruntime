...
class GraphExecutionManager(GraphExecutionInterface):
    ...
    def _add_check_embedding_sparsity_hook(self):
        ...
        def _embedding_hook(name, module, args):
            ...
            if embed_density < 90:
                ...
                return FlagPaddingElimination.apply(args[0])
            else:
    ...
    def _add_check_label_sparsity_hook(self):
        ...
        """
        if not self._runtime_options.enable_sparse_optimizer:
            return None
        ...
        def _label_hook(name, module, args):
            ...
            if label_density < 90:
                ...
                return (args[0], FlagPaddingElimination.apply(args[1]))
            else:
            ...
            if self._runtime_options.enable_sparse_optimizer:
                ...
                if self._runtime_inspector._sceloss_module_to_ignore_density_map:
                    self._runtime_options.label_sparsity_ratio = ",".join(
                        [
                            f"{k}:{v:.0f}%"
                            for k, v in self._runtime_inspector._sceloss_module_to_ignore_density_map.items()
                        ]
                    )
                ...
                if self._runtime_inspector._embedding_module_to_padding_density_map:
                    self._runtime_options.embed_sparsity_ratio = ",".join(
                        [
                            f"{k}:{v:.0f}%"
                            for k,v in self._runtime_inspector._embedding_module_to_padding_density_map.items()
                        ]
                    )
