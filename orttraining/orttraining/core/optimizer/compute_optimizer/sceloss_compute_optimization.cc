...
constexpr const char* kFlagSparseFuncName =
  ...
  for (const auto index : order) {
    ...
    if (labels_producer == nullptr ||
      ...
      LOG_DEBUG_INFO(logger, "Skip node " + node.Name() + "(" + node.OpType() +
                                 ...
                                 kFlagSparseFuncName + ".");
      std::cout<<"yangyangyang cannot find the python op node"<<std::endl;
      continue;
    } else {
      std::cout<<"yangyangyang find the python op node"<<std::endl;
      if (graph_utils::CanRemoveNode(graph, *labels_producer, logger)) {
        if (graph_utils::RemoveNode(graph, *labels_producer)) {
          std::cout<<"yangyangyang remove the python op node"<<std::endl;
          modified = true;
        } else {
      ...
      // label_input_arg = node.InputDefs()[1];
      // labels_producer = graph.GetMutableProducerNode(label_input_arg->Name());
    }
